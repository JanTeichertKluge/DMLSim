...
class CrossEmbeddings:
  ...
  def slice_input(self, idx):
    inputs = {'input_ids' : self.txt_tokens['input_ids'][idx],
  ...
  def fit_and_predict_embeddings(self):
    self._prepare_data()
    for fold, (train_idx, output_idx) in enumerate(KFold(n_splits=self.n_folds, shuffle=True, random_state=1234).split(self.dataset)):
      print(f"Fold {fold+1} from {self.n_folds}")
      aux_y_tmp = clone(self.aux_y)
      aux_d_tmp = clone(self.aux_d)
      y_tmp = self.dataset.loc[train_idx, self.y_col].to_numpy().reshape(-1,1).astype(np.float32)
      d_tmp = self.dataset.loc[train_idx, self.d_col].to_numpy().reshape(-1,1).astype(np.float32)
