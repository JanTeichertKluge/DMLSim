...
class network_builder:
    ...
    def __init__(self, 
                 ...
                 layer_sizes={
                  ...
                  'model2': [80, 80, 80],
                  'model3': [60, 30, 20, 10],
                  'model4': [20, 15, 15, 10, 10, 5],
                  'model5': [60, 30, 20, 20, 10, 5]}, 
                 dropout=0.2, 
                 lr=0.001, 
                 batch_size=128, 
                 n_epochs=100, 
    ...
    def get_plr_nn_learners(self, discrete_treatment = False):
        ...
        learner_dict_plr_nn = {f"neural_net_{nn_name}": {'ml_l' : clone(nn_reg_skorch_models[nn_name]), 
                                                         'ml_m' : clone(nn_cls_skorch_models[nn_name] if discrete_treatment else nn_reg_skorch_models[nn_name]),
                                                         'ml_g' : clone(nn_reg_skorch_models[nn_name])} for nn_name in nn_reg_skorch_models.keys()}
    ...
    def get_irm_nn_learners(self):
        ...
        learner_dict_irm_nn = {f"neural_net_{nn_name}": {'ml_m' : clone(nn_cls_skorch_models[nn_name]),
                                                         'ml_g' : clone(nn_reg_skorch_models[nn_name])} for nn_name in nn_reg_skorch_models.keys()}
