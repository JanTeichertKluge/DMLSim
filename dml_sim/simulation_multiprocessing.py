...
# New classes needed for multiprocessing
...
class simulation_study_mp:
    ...
    -------
        ...
        _create_dml_data_obj()
            Create and return a DoubleMLData object from arrays.
        _create_dml_object(obj_dml_data)
            Create and return a DoubleML object with the given DoubleMLData object.
        _run_fit()
            Performs a fit of a DoubleML object and saves the results.
        run_simulation()
        ...
        _absolute_bias(theta_dml_i, theta_0)
            Calculate the average absolute bias.
        _relative_bias(theta_dml_i, theta_0)
            Calculate the relative bias.
        _standardized_bias(theta_dml_i, theta_0, se_dml_i)
            Calculate the standardized bias.
        _rmse(theta_dml_i, theta_0)
            Calculate the root mean squared error.
        _average_se(se_dml_i)
            Calculate the average standard error.
        _empirical_deviation(theta_dml_i)
            Calculate the empirical deviation.
        _coverage(lowCI_i, upCI_i, theta_0)
            Calculate the coverage of the confidence intervals.
        iterate_performance_measures()
    ...
    _model_attr = ["theta_dml", "se_dml", "lowCI", "upCI"]
    ...
    def __init__(
        ...
        self._model_cache = {
            attr: np.zeros(shape=(n_rep,)) for attr in self._model_attr
        }
        self.model_attr = {
            attr: {key: {} for key in self.lrn_dict.keys()} for attr in self._model_attr
        }
        ...
        self.performance_cache = {
            pm: {key: {} for key in self.lrn_dict.keys()}
            for pm in self._performance_measures
        }
        self.performance_df = None
        self.histograms = {}
        self.boxplots = {}
        self._nworkers = multiprocessing.cpu_count()
        ...
        for lrnr in self.lrn_dict.values():
            if (
                ...
                and self.score == "IV-type"
            ):
                if not (
                    check_key(lrnr, "ml_g")
                    and check_key(lrnr, "ml_m")
                    and check_key(lrnr, "ml_l")
                ):
                    raise ValueError(
                        "At least one learner is missing in your learner dict. Please check learner dictionary."
                    )
            elif (
                self.model == doubleml.double_ml_plr.DoubleMLPLR
            ...
            elif self.model == doubleml.double_ml_irm.DoubleMLIRM and (
                self.score == "ATE" or self.score == "ATTE"
            ):
                ...
                raise ValueError(
                    "The given score function does not match to the given model. Please check model and score type."
                )
    ...
    def _prepare_data(self, setting):
        ...
        if not self.is_heterogenous or (
            ...
            for _ in range(self.n_rep):
                ...
                self._data.append((x, y, d))
                if self.score == "ATE":
                    if all(treatment_eff[d == 0] == d[d == 0]):
                        self.theta_0[setting].append(
                            np.mean(treatment_eff[d == 1])
                        )  # according to opossum DGPs data structure
                    else:
                    ...
                    print(
                        "The given score function does not match to the defined model setting."
                    )
    ...
    def _run_fit(self, rs):
        ...
        return {
            "theta_dml": obj_dml.coef[0],
            "se_dml": obj_dml.se[0],
            "lowCI": obj_dml.confint()["2.5 %"][0],
            "upCI": obj_dml.confint()["97.5 %"][0],
        }
            ...
            for self._dim_x_act in self.np_dict["dim_x"]:
                ...
                for lvl_lrn_dict in self.lrn_dict.keys():
                    ...
                    random_state = np.random.randint(
                        np.iinfo(np.int32).max, size=self.n_rep
                    )
                    ...
                    self.model_attr["theta_dml"][self._lrn_act][indx] = np.array(
                        self._model_cache["theta_dml"]
                    )
                    self.model_attr["se_dml"][self._lrn_act][indx] = np.array(
                        self._model_cache["se_dml"]
                    )
                    self.model_attr["lowCI"][self._lrn_act][indx] = np.array(
                        self._model_cache["lowCI"]
                    )
                    self.model_attr["upCI"][self._lrn_act][indx] = np.array(
                        self._model_cache["upCI"]
                    )
                    self._i_rep = 0
    ...
    def iterate_performance_measures(self):
        ...
        for learner_i in self.lrn_dict.keys():
            for setting in self._all_permutations:
                ...
                self.performance_cache["abs_bias"][learner_i][
                    setting
                ] = self._absolute_bias(
                    self.model_attr["theta_dml"][learner_i][setting], theta_0
                )
                self.performance_cache["rel_bias"][learner_i][
                    setting
                ] = self._relative_bias(
                    self.model_attr["theta_dml"][learner_i][setting], theta_0
                )
                self.performance_cache["std_bias"][learner_i][
                    setting
                ] = self._standardized_bias(
