...
class simulation_study_mp:
    ...
    def _run_fit(self, rs):
        ...
        return {"theta_dml":obj_dml.coef[0], 
                "se_dml":obj_dml.se[0], 
            ...
            for self._dim_x_act in self.np_dict["dim_x"]:
                ...
                for lvl_lrn_dict in self.lrn_dict.keys():
                    ...
                    self.model_attr["theta_dml"][self._lrn_act][
                        indx
                    ] = np.array(self._model_cache["theta_dml"])
                    self.model_attr["se_dml"][self._lrn_act][indx] = np.array(self._model_cache[
                        "se_dml"
                    ])
                    self.model_attr["lowCI"][self._lrn_act][indx] = np.array(self._model_cache[
                        "lowCI"
                    ])
                    self.model_attr["upCI"][self._lrn_act][indx] = np.array(self._model_cache[
                        "upCI"
                    ])
                    self._i_rep = 0
